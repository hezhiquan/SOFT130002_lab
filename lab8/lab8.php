...
function sub(){
    $all=0;
    for($i=1;$i<5;$i++){
       $all+=$GLOBALS['price'.$i]*$GLOBALS['quantity'.$i];
    }
    return $all;
}
$subtotal=sub();
function shipping(){
    $GLOBALS['shipping']=$GLOBALS['subtotal']>$GLOBALS['shippingThreshold']?100:$GLOBALS['shippingFlatAmount'];
}
shipping();
function grand(){
    $GLOBALS['grandTotal']=$GLOBALS['shipping']+$GLOBALS['subtotal'];
}
grand();
?>
